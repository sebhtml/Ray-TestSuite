#!/bin/bash
#PBS -N human-genome-2012-09-25-4096.1
#PBS -q qtest@mp2
#PBS -l nodes=171:ppn=24
...
# Date: 2012-09-25
# Tester: Sébastien Boisvert

# Resource: mammouth mp2
# Processor: AMD Opteron 6172
# Kernel: Linux 2.6.32-131.21.1.el6.x86_64
# Page size: 4096 bytes
# Interconnect: Mellanox MT26428 revision b0
# Compiler: gcc 4.7.0
# MPI library: Open-MPI 1.5.4
# Middleware: OFED 1.4.1

# Dataset: SRA000271 (4045335994 sequences of 36 nucleotides)
# Sequencing technology: Illumina Genome Analyzer

# Application: Ray v2.1.0-devel 2ee566a095
# Framework: RayPlatform v1.1.0-devel 9cd8ac7867d9
# Processes: 4096
# Communication pattern: virtual is any-to-any, physical is the surface of a polytope
# Previous similar tests: 4096 Ray processes on the Cray XE6

#</DataSheet>

# mammouth is a fancy resource => https://rqchp.ca/?pageId=1388
# it has 1588 machines
# each machine has 2 processors
# each processor has 12 processor cores
# it has 39648 processor cores

# the job requests 171 machines
# this is 4104 processor cores
# but we will use only 4096 because of our fancy
# virtual router in RayPlatform
# therefore, all the machines will have 24 or 23 Ray processes

# 171*24 = 4104

# for the virtual routing, we will use the polytope as it
# is simply the best communication graph with its dynamic load
# balancing

# so now we need to choose a degree.
# some of the choices are below:

# 8^4 = 4096 => polytope with degree 4*(8-1)=28
# 16^3 = 4096 => polytope with degree 3*(16-1)=45
# 64^2 = 4096 => polytope with degree 2*(64-1)=126

# so the degree of the polytope is 28 or 45 or 126
# We will use a degree of 28 for no reason particularly.

# k=27 in http://genome.cshlp.org/content/19/6/1117.full

. /mnt/scratch_mp2/corbeil/corbeil_group/software/NGS-Pipelines/LoadModules.sh

mpiexec -output-filename human-genome-2012-09-25-4096.1 \
-bynode \
-n 4096 \
Ray \
-o human-genome-2012-09-25-4096.1 \
-k 27 \
-route-messages -connection-type polytope -routing-graph-degree 28 \
-p \
